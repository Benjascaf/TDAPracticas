...
public:
    ...
    std::list<std::tuple<int, int>> getAdjacent(int v)
    {
        return m_g[v];
    }
...
std::tuple<std::vector<int>, std::vector<float>> bfs(Graph g, int s)
{
    std::vector<int> nodes{g.getNodes()};
    // Los colores van a estar codeados por numeros, 0 = blanco, -1 para gris, y 1 para negro
    std::vector<int> colours{std::vector<int>(nodes.size(), 0)};
    float inf{std::numeric_limits<float>::infinity()};
    std::vector<float> distances{std::vector<float>(nodes.size(), inf)};
    std::vector<int> parents{std::vector<int>(nodes.size(), 0)};
    colours[s] = -1;
    distances[s] = 0;
    std::queue<int> q;
    q.push(s);
    while (!q.empty())
    {
        int u = q.front();
        q.pop();
        for (std::tuple<int, int> w : g.getAdjacent(u))
        {
            int wp = get<0>(w);
            if (colours[wp] == 0)
            {
                colours[wp] = -1;
                distances[wp] = distances[u] + 1;
                parents[wp] = u;
                q.push(wp);
            }
        }
    ...
    return std::tuple<std::vector<int>, std::vector<float>>(parents, distances);
}
...
void dfs(Graph g)
{
    ...
    std::vector<int> colours{std::vector<int>(nodes.size(), 0)};
    std::vector<int> parents{std::vector<int>(nodes.size(), 0)};
    std::vector<std::tuple<int, int>> times{std::vector<std::tuple<int, int>>(nodes.size())};
    int time = 0;
}
